...
def memcached(setup: str, preset_rates, dataroot):
    ...
    for i in range(1, 4+1):
        ...
        xl_cpupool_migrate(f"seastar{i}", setup)

    time.sleep(1)

    logdir = f"{dataroot}/memcached/{setup}"
...
def nginx(setup: str, preset_rates, dataroot):
    for i in range(1, 4+1):
        xl_create(f"fstack{i}")

    for i in range(1, 4+1):
        host = f"root@192.168.70.{50+i}"
        wait_domain(host)
        # vfiobind(host)
        ssh_popen(host, f"/usr/local/nginx_fstack/sbin/nginx")
        xl_cpupool_migrate(f"fstack{i}", setup)

    for i in range(1, 4+1):
        while client_popen(f"ping -c 1 -W 1 10.0.0.{10+i}").wait(): time.sleep(1)

    logdir = f"{dataroot}/nginx/{setup}"
    subprocess.run(f"mkdir -p {logdir}", shell=True, check=True)
    ...
    for rate in rates:
        assert(client_popen(f"~/wrk2/wrk http://10.0.0.11 --duration 15 --threads 24 --connections 24 --rate {rate} --u_latency | tee {logdir}/{rate}rps").wait() == 0)

    subprocess.run("./reset.sh", shell=True)


def redis(setup: str, preset_rates, dataroot):
    for i in range(1, 4+1):
        xl_create(f"fstack{i}")
    ...
    for rate in rates:
        assert(client_popen(f"~/memtier_benchmark/memtier_benchmark --host 10.0.0.11 --port 6379 --test-time 10 --ratio 0:1 --threads 24 --clients 1 --rate {rate} | tee {logdir}/{rate}rps").wait() == 0)

    subprocess.run("./reset.sh", shell=True)

def with_credit(func, rates, dataroot):
    pool_vanilla.credit(tslice_ms=1, ratelimit_us=1000, cpus=[0])
    func("credit", rates, dataroot)
def with_credit2(func, rates, dataroot):
    pool_vanilla.credit2(ratelimit_us=0, cpus=[0])
    func("credit2", rates, dataroot)
...
if __name__ == "__main__":
    ...
    if "memcached" in workloads:
        with_credit(memcached, args.rates, dataroot)
        with_credit2(memcached, args.rates, dataroot)
    if "nginx" in workloads:
        with_credit(nginx, args.rates, dataroot)
        with_credit2(nginx, args.rates, dataroot)
    if "redis" in workloads:
        with_credit(redis, args.rates, dataroot)
        with_credit2(redis, args.rates, dataroot)
